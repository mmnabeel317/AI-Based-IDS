...
class IDSApplication(QMainWindow):
    ...
    @pyqtSlot(dict)
    def on_flow_detected(self, result: dict):
        """
        Callback when flow is detected and analyzed.
        Thread-safe - called via Qt signal from worker thread.
        
        Args:
            result: Analysis result dictionary
        """
        try:
            # Add to table (this is now in GUI thread)
            self.flow_table.add_flow(result)
            
            # Show alert for high-severity threats
            if result.get('alert_level') in ['high', 'critical']:
                self.show_threat_alert(result)
