...
class CaptureController:
    ...
    def analyze_pcap(self, pcap_path, callback=None):
        """
        Analyze PCAP file offline.
        
        Args:
            pcap_path (str): Path to PCAP file
            callback (callable): Callback for each completed flow
        
        Returns:
            list: All flows with predictions
        """
        try:
            ...
            # Load PCAP
            pcap_loader = PCAPLoader(pcap_path)
            packets = pcap_loader.read_packets()
            ...
            for i, packet_dict in enumerate(packets):
                completed_flow = flow_builder.add_packet(packet_dict)
                
                if completed_flow:
                    completed_flows.append(completed_flow)
                
                if i % 100 == 0 and i > 0:
                    flushed = flow_builder.flush_inactive_flows()
                    completed_flows.extend(flushed)
            
            # Finalize
            final_flows = flow_builder.finalize()
            
            logger.info(f"Built {len(final_flows)} flows")
            
            # Predict and emit results
            results = []
            
            for i, flow in enumerate(final_flows):
                # Make prediction
                prediction = self.predictor.predict(flow)
                
                # Get Suricata result (check if components exist)
                suricata_result = None
                if (self.suricata_enabled and 
                    hasattr(self, 'suricata_parser') and self.suricata_parser and
                    hasattr(self, 'suricata_correlator') and self.suricata_correlator):
                    try:
                        suricata_result = self.suricata_correlator.correlate(flow, [])
                    except Exception as e:
                        logger.debug(f"Suricata correlation failed: {e}")
                ...
                # Log progress
                if i > 0 and i % 100 == 0:
                    logger.info(f"Analyzed {i}/{len(final_flows)} flows")
            
            logger.info(f"Analyzed {len(results)} flows")
